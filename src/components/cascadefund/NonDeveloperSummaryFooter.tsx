...
/** Closing CTA + links for non-developer landing copy (waitlist modal, RPG focus, landing link). */
export function NonDeveloperSummaryFooter() {
  return (
    <>
      <p className="flex flex-wrap items-center gap-x-1.5 gap-y-2 text-left text-sm leading-relaxed md:text-base">
        ...
        <JoinWaitlistDialogRegistration
          dialogList={[]}
          triggerLabel="Join the waitlist now"
        ...
        <button
          ...
          title="Show dialogue"
          onClick={() => {
            setCascadeFundUserSummaryExpanded(false)
            requestAnimationFrame(() => {
              document.querySelector('[aria-label="Dialogue"]')?.scrollIntoView({ block: 'end', behavior: 'smooth' })
            })
          }}
          className={cn(
