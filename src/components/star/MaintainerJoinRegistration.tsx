            ...
            >
              ...
              >
                <div className="relative flex shrink-0 items-start gap-3 border-b border-slate-400/25 pb-3 dark:border-white/10">
                  ...
                  >
                    {phase === 'form' ? 'Join CascadeFund' : 'Thank you'}
                  </h2>
                  <button
                    type="button"
                    onClick={resetAndClose}
                  ...
                  {phase === 'form' ? (
                    ...
                    >
                      <div className="space-y-1.5">
                        <label htmlFor="maintainer-join-email" className="text-sm font-medium text-slate-700 dark:text-slate-300">
                          Email
                        </label>
                        <input
                          id="maintainer-join-email"
                          type="email"
                          autoComplete="email"
                          value={email}
                      ...
                      <div className="space-y-1.5">
                        <label htmlFor="maintainer-join-project-url" className="text-sm font-medium text-slate-700 dark:text-slate-300">
                          Project URL
                        </label>
                        <p className="text-xs text-slate-500 dark:text-slate-400">Public Git repository URL (GitHub, GitLab, etc.)</p>
                        <input
                          id="maintainer-join-project-url"
                          type="url"
                          autoComplete="url"
                          value={projectUrl}
                      ...
                      <label className="flex cursor-pointer items-start gap-3 text-sm leading-snug text-slate-700 dark:text-slate-300">
                        <input
                          type="checkbox"
                          checked={allowSocialTag}
                          onChange={(e) => setAllowSocialTag(e.target.checked)}
                          className={cn(
                            'mt-0.5 h-4 w-4 shrink-0 rounded border-slate-400 text-sky-600',
                            'focus:ring-2 focus:ring-sky-500/40 dark:border-slate-500',
                          )}
                        />
                        <span>
                    ...
                    <div className="flex flex-col gap-6">
                      <p className="text-sm leading-relaxed text-slate-700 dark:text-slate-200 sm:text-base">
                        Thanks for registering, keep an eye on email, I will message you soon. :)
                      </p>
